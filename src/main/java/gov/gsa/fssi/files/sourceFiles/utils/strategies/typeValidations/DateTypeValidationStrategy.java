	...
	public void validate(SchemaField field, Data data) {
		if(data != null){
			if(data.getData() != null && !data.getData().isEmpty()){
				...
				if(date == null){
					data.addValidationResult(false, 3, "Type("+field.getType()+")"); //Fatal error, could not find date
					...
					data.addValidationResult(false, 2, "Type("+field.getType()+")"); //Only an error if out of bounds
