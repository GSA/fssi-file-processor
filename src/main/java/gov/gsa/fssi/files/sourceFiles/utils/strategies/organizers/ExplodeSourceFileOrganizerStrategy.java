		...
		while (currentHeaderIterator.hasNext()) {
			...
			while (headerTranslationMapIterator.hasNext()) {
				...
				if (!foundColumn){
					...
					logger.info("Source field '{} - {}' was not in Schema, but thats ok!, adding to newHeader index '{}'", currentHeaderIteratorPairs.getValue().toString().toUpperCase(), currentHeaderIteratorPairs.getKey(),  headerCounter);
					headerTranslationMap.put(currentHeaderIteratorPairs.getKey(), headerCounter);
					newHeader.put(headerCounter, currentHeaderIteratorPairs.getValue());
					headerCounter ++;	
				}
		}
